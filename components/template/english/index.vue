        ...
        <template #config>
            <div class="sm:flex items-center justify-between">
                <el-form class="flex items-center flex-wrap">
                    ...
                    <el-form-item label="字体" class="w-32 mr-4">
                        <el-select
                            v-model="currentFontStyleIndex"
                            placeholder=""
                            @change="changeSize"
                        >
                            <el-option
...
// 配置项
const config = [
    { label: '较小 - 行高0.96cm', size: 'sm', maxRows: 16, maxCols: 14 },
    { label: '适中 - 行高1.20cm', size: 'base', maxRows: 13, maxCols: 12 },
    { label: '较大 - 行高1.50cm', size: 'lg', maxRows: 11, maxCols: 12 },
    { label: '巨大 - 行高1.80cm', size: 'xl', maxRows: 10, maxCols: 10 }
]
const currentConfigIndex = useCookie('template_english_current_config') // 当前配置项索引
currentConfigIndex.value =
    currentConfigIndex.value === undefined ? 1 : currentConfigIndex.value
const currentConfig = computed(() => config[currentConfigIndex.value])
const currentSize = computed(() => currentConfig.value.size) // 当前尺寸
const rows = useCookie('template_english_rows') // 行数
rows.value = rows.value || currentConfig.value.maxRows
const cols = useCookie('template_english_cols') // 列数
cols.value = cols.value || currentConfig.value.maxCols

const fontStyles = [
    { label: 'Italic', key: 'italic' },
    { label: 'SegoeUI Italic', key: 'segoe' }
]
